...
#define ERR(S, ...)                                                            \
 printf(ColorError "[ERROR] %s:%d -> " S ColorReset "\n", __FILE__, __LINE__,  \
        __VA_ARGS__)
#define WARN(S, ...)                                                           \
 printf(ColorWarn "[WARN]  %s:%d -> " S ColorReset "\n", __FILE__, __LINE__,   \
        __VA_ARGS__)
#define DEBUG(S, ...)                                                          \
 printf(ColorDebug "[DEBUG] %s:%d -> " S ColorReset "\n", __FILE__, __LINE__,  \
        __VA_ARGS__)
#define INFO(S, ...)                                                           \
 printf(ColorInfo "[INFO]  %s:%d -> " S ColorReset "\n", __FILE__, __LINE__,   \
        __VA_ARGS__)
...
/*~ MEMORY */
typedef void *
MM_ReserveFunc(void *ctx, u64 size);
typedef void
MM_ChangeMemoryFunc(void *ctx, void *ptr, u64 size);
...
/* UTILITIES */
internal void *
mm_memset(void *dest, int val, u64 len);
internal void *
...
internal void
mm_change_memory_noop(void *ctx, void *ptr, u64 size)
{
}
internal Arena
mm_make_arena_reserve(MM_BaseMemory *base, u64 reserve_size);
internal Arena
mm_make_arena(MM_BaseMemory *base);
internal Arena
mm_scratch_arena();

internal void
mm_arena_release(Arena *arena);
internal void *
mm_arena_push(Arena *arena, u64 size);
internal void *
mm_arena_push_zeros(Arena *arena, u64 size);
internal void
mm_arena_pop_to(Arena *arena, u64 pos);
